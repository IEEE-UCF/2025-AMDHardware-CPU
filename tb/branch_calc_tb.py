...
# RISC-V RV64I ISA Specification-based Branch Calculation Test
# Tests branch address calculation according to RISC-V ISA specification


class RV64IBranchSpec:
    """RISC-V RV64I Branch and Jump Address Calculation Specification"""

    # RISC-V RV64I Opcodes for control flow instructions
    ...
    def extract_b_immediate(instruction):
        ...
        # Sign extend from 13 bits to 64 bits
    ...
    def calculate_branch_address(pc, instruction):
        ...
        imm = RV64IBranchSpec.extract_b_immediate(instruction)
        return (pc + imm) & 0xFFFFFFFFFFFFFFFF

    @staticmethod
    def calculate_jal_address(pc, instruction):
        """Calculate JAL target address per RISC-V ISA"""
        imm = RV64IBranchSpec.extract_j_immediate(instruction)
        return (pc + imm) & 0xFFFFFFFFFFFFFFFF

    @staticmethod
    def calculate_jalr_address(data_a, instruction):
        """Calculate JALR target address per RISC-V ISA"""
        imm = RV64IBranchSpec.extract_i_immediate(instruction)
        # JALR sets LSB to 0 per ISA specification
        target = (data_a + imm) & 0xFFFFFFFFFFFFFFFE
        return target & 0xFFFFFFFFFFFFFFFF


@cocotb.test()
async def test_risc_v_branch_address_calculation(dut):
    """Test branch address calculation compliance with RISC-V RV64I ISA"""

    spec = RV64IBranchSpec()
    dut._log.info("=== Testing RISC-V RV64I Branch Address Calculation ===")
    ...
    for funct3, name in branch_ops:
        for i in range(num_tests_per_type):
            ...
            dut_bra_addr = int(dut.bra_addr.value) & 0xFFFFFFFFFFFFFFFF
            ...
            if dut_bra_addr == expected_bra_addr:
                ...
                dut._log.error(f"  Expected bra_addr: 0x{expected_bra_addr:016x}")
                dut._log.error(f"  Got bra_addr:      0x{dut_bra_addr:016x}")
    ...
    for i in range(num_tests_per_type):
        ...
        dut_jal_addr = int(dut.jal_addr.value) & 0xFFFFFFFFFFFFFFFF
        ...
        if dut_jal_addr == expected_jal_addr:
            ...
            dut._log.error(f"  PC: 0x{pc:08x}")
            dut._log.error(f"  Instruction: 0x{instruction:08x}")
            dut._log.error(f"  Expected jal_addr: 0x{expected_jal_addr:016x}")
            dut._log.error(f"  Got jal_addr:      0x{dut_jal_addr:016x}")
    ...
    for i in range(num_tests_per_type):
        ...
        rd = random.randint(0, 31)
        rs1 = random.randint(0, 31)
        data_a = random.randint(0, 0xFFFFFFFFFFFFFFFF)  # Base register value
        ...
        dut_jalr_addr = int(dut.jalr_addr.value) & 0xFFFFFFFFFFFFFFFF
        ...
        if dut_jalr_addr == expected_jalr_addr:
            ...
            dut._log.error(f"  PC: 0x{pc:08x}")
            dut._log.error(f"  Instruction: 0x{instruction:08x}")
            dut._log.error(f"  data_a: 0x{data_a:016x}")
            dut._log.error(f"  Expected jalr_addr: 0x{expected_jalr_addr:016x}")
            dut._log.error(f"  Got jalr_addr:      0x{dut_jalr_addr:016x}")
            dut._log.error(f"  Immediate: {imm} (0x{imm & 0xFFF:03x})")
            dut._log.error(
                f"  Raw calculation: 0x{(data_a + imm):016x} -> 0x{expected_jalr_addr:016x}"
            )
    ...
    if failed_tests == 0:
        dut._log.info("✓ Branch calculation module is RISC-V RV64I ISA compliant!")
    else:
...
async def test_specific_risc_v_cases(dut):
    ...
    assert int(dut.bra_addr.value) == expected_addr, (
        f"BEQ forward branch: expected 0x{expected_addr:x}, got 0x{int(dut.bra_addr.value):x}"
    )
    ...
    expected_addr = (pc - 12) & 0xFFFFFFFFFFFFFFFF
    ...
    assert int(dut.bra_addr.value) == expected_addr, (
        f"BNE backward branch: expected 0x{expected_addr:x}, got 0x{int(dut.bra_addr.value):x}"
    )
    ...
    assert int(dut.jal_addr.value) == expected_addr, (
        f"JAL forward jump: expected 0x{expected_addr:x}, got 0x{int(dut.jal_addr.value):x}"
    )
    ...
    expected_addr = (pc - 2048) & 0xFFFFFFFFFFFFFFFF
    ...
    assert int(dut.jal_addr.value) == expected_addr, (
        f"JAL backward jump: expected 0x{expected_addr:x}, got 0x{int(dut.jal_addr.value):x}"
    )
    ...
    expected_addr = (base_addr + 100) & 0xFFFFFFFFFFFFFFFE  # Clear LSB per RISC-V spec
    ...
    assert int(dut.jalr_addr.value) == expected_addr, (
        f"JALR positive offset: expected 0x{expected_addr:x}, got 0x{int(dut.jalr_addr.value):x}"
    )

    # Test case 6: JALR with negative offset
    # JALR x7, x15, -500  (jump to x15 - 500, clear LSB)
    pc = 0x6000
    base_addr = 0xFFFFFFFFABCDEF00
    instruction = spec.encode_i_type(spec.OP_JALR, 7, 0b000, 15, -500)
    expected_addr = ((base_addr - 500) & 0xFFFFFFFFFFFFFFFF) & 0xFFFFFFFFFFFFFFFE
    ...
    assert int(dut.jalr_addr.value) == expected_addr, (
        f"JALR negative offset: expected 0x{expected_addr:x}, got 0x{int(dut.jalr_addr.value):x}"
    )
    ...
    expected_addr = base_addr & 0xFFFFFFFFFFFFFFFE  # Should clear LSB
    ...
    assert int(dut.jalr_addr.value) == expected_addr, (
        f"JALR LSB clearing: expected 0x{expected_addr:x}, got 0x{int(dut.jalr_addr.value):x}"
    )
...
async def test_edge_cases(dut):
    ...
    assert int(dut.bra_addr.value) == expected_addr, (
        f"Max positive branch: expected 0x{expected_addr:x}, got 0x{int(dut.bra_addr.value):x}"
    )
    ...
    expected_addr = (pc + min_branch_offset) & 0xFFFFFFFFFFFFFFFF
    ...
    assert int(dut.bra_addr.value) == expected_addr, (
        f"Max negative branch: expected 0x{expected_addr:x}, got 0x{int(dut.bra_addr.value):x}"
    )
    ...
    assert int(dut.jal_addr.value) == expected_addr, (
        f"Max positive JAL: expected 0x{expected_addr:x}, got 0x{int(dut.jal_addr.value):x}"
    )
    ...
    expected_addr = (pc + min_jal_offset) & 0xFFFFFFFFFFFFFFFF
    ...
    assert int(dut.jal_addr.value) == expected_addr, (
        f"Max negative JAL: expected 0x{expected_addr:x}, got 0x{int(dut.jal_addr.value):x}"
    )
